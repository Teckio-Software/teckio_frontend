...
import { precioUnitarioDetalleCopiaDTO, precioUnitarioDetalleDTO } from '../../precio-unitario-detalle/tsPrecioUnitarioDetalle';
import { precioUnitarioDTO } from '../tsPrecioUnitario';
import { InsumoDTO } from 'src/app/catalogos/insumo/tsInsumo';

@Component({
  selector: 'app-matriz-precio-unitario-copia',
  templateUrl: './matriz-precio-unitario-copia.component.html',
  styleUrls: ['./matriz-precio-unitario-copia.component.css']
})
export class MatrizPrecioUnitarioCopiaComponent implements OnInit {
  @Input() selectedPU!: number;
  selectedProyecto: number = 0;
  selectedEmpresa: number = 0;
  nivelDesglose: number = 0;
  Unidades: string[] = [];
  detalles!: precioUnitarioDetalleCopiaDTO[];
  detallesReset!: precioUnitarioDetalleCopiaDTO[];
  tipoInsumo!: tipoInsumoDTO[];
  familiaInsumo!: familiaInsumoDTO[];
  varDetalleSeleccionado!: precioUnitarioDetalleCopiaDTO;
  ...
  cargarDetallesXIdPrecioUnitario() {
    ...
    let detalleVacio: precioUnitarioDetalleCopiaDTO = {
      id: 0,
      idPrecioUnitario: 0,
      idInsumo: 0,
      esCompuesto: false,
  ...
  exponerNumeros(detalle: precioUnitarioDetalleCopiaDTO){
  ...
  detallesDesgloce: precioUnitarioDetalleCopiaDTO[] = [];

  cargarRegistroDelDesglose(desglose: precioUnitarioDetalleCopiaDTO) {
    console.log(desglose);
      if (desglose.id == 0) {
        ...
        let Detalle: precioUnitarioDetalleCopiaDTO = {
