...
import { polizaDTO } from '../../tsPoliza';
import { polizaDetalleDTO } from 'src/app/contabilidad/poliza-detalle/tsPolizaDetalle';
import { PolizaDetalleService } from 'src/app/contabilidad/poliza-detalle/poliza-detalle.service';
...
export class PolizaModalComponent {
  ...
  @Input() poliza!: polizaDTO;
  @Input() polizasDetalles: polizaDetalleDTO[] = [];
  @Input() totalDebe: number = 0;
  @Input() totalHaber: number = 0;
  ...
  generarPdf() {
    ...
    const fecha = format(
      new Date(this.poliza.fechaPoliza),
      "dd 'de' MMMM 'de' yyyy",
      { locale: es }
    );
      ...
      columns: [
        {
          stack: [
            {
              text: [
                { text: 'Poliza No.: ', bold: true },
                { text: `${this.poliza.folio}` },
              ],
              alignment: 'left',
              style: 'header',
            },
    ...
    const tableBodyProject = [
      [
        { text: 'Cuenta contable', style: 'subheader' },
        { text: 'Descripción', style: 'subheader' },
        { text: 'Cargo', style: 'subheader' },
        { text: 'Abono', style: 'subheader' },
      ],
      ...this.polizasDetalles.map((data) => {
        return [
          { text: data.cuentaContableCodigo, style: 'small' },
          { text: data.concepto, style: 'small' },
          { text: data.debe, style: 'small' },
          { text: data.haber, style: 'small' },
        ];
      }),
    ];
    ...
    const numeroDeFilas = tableBodyProject.length;
              ...
              table: {
                ...
                body: [
                  [
                    ...
                    { text: `${this.totalDebe}`, style: 'smallBold' },
                    { text: `${this.totalHaber}`, style: 'smallBold' },
                  ],
                ],
              },
            },
    ...
    pdfMake
      ...
      .download(this.poliza.folio + '_' + this.poliza.concepto);
